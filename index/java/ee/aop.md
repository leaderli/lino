...
### 连接点
`Join Point` java 运行过程中的位置，对于`Spring`来说，表示允许使用切面的位置，它一般是指方法运行的过程，比如运行前，运行后，抛出异常等。
### 通知
`Advice` 在某连接点上执行的特殊操作
### 切入点
`PointCut` 切入点通过表达式来连接通知和连接点，确定连接点是否符合切入点表达式
### 目标类
`Target` 被切面切入的类
### 代理类
`AOP proxy` 实现切面的方式即实现目标类的代理类对象，对于`Spring`来说，通过`JDK`动态代理和`CGLIB`字节码代理
### 织入
