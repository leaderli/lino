---
tags:
  - java/spring/ResourceHandlers
  - source
date updated: 2022-04-17 20:27
---
...
`Spring` 拦截 url，根据 `HandlerMapping` 找到对应的 `Handler` 去执行相关操作。
...
`Spring` 的请求由 `DispatcherServlet` 拦截所有请求，实现了 `Servlet` 标准。那么我们从 `service`方法入口即可
