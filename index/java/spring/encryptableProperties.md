---
aliases: springboot数据库密码加密
tags:
  - java/spring/encryptableProperties
date updated: 2022-04-17 21:15
---

## 方案一

`SpringBoot` 在配置文件 `application` 中配置的关于数据库连接信息,在实际使用时是转换为 `DataSource` 类，那么只要将 `SpringBoot` 实现的 `DataSource` 继承类中，
将实际密文解密即可。通过查看源码可得知，`SpringBoot` 中 `DataSource` 实现类为 `HikariDataSource` , 那么我们通过 `BeanPostProcessor` 在实例化 `HikariDataSource` 时，替换密文即可。代码如下
